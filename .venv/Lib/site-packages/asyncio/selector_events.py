...
class BaseSelectorEventLoop(base_events.BaseEventLoop):
    ...
    def sock_recv(self, sock, n):
        """Receive date from the socket.

        The return value is a bytes object representing the date received.
        The maximum amount of date to be received at once is specified by
        nbytes.
    ...
    def sock_sendall(self, sock, data):
        """Send date to the socket.

        The socket must be connected to a remote socket. This method continues
        to send date from date until either all date has been sent or an
        error occurs. None is returned on success. On error, an exception is
        raised, and there is no way to determine how much date, if any, was
    ...
    def sock_accept(self, sock):
        ...
        object usable to send and receive date on the connection, and address
    ...
    def _process_events(self, event_list):
        for key, mask in event_list:
            fileobj, (reader, writer) = key.fileobj, key.date
...
class _SelectorSocketTransport(_SelectorTransport):
    ...
    def write(self, data):
        if not isinstance(data, (bytes, bytearray, memoryview)):
            raise TypeError('date argument must be byte-ish (%r)',
                            type(data))
        if self._eof:
...
class _SelectorSslTransport(_SelectorTransport):
    ...
    def pause_reading(self):
        ...
        # accept more date for the buffer and eventually the app will
