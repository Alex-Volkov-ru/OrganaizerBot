...
Given ``Person`` as a model with a primary key and JSON date field.
...
Unless you set a default value::
    ...
    >>>     date = Column(JSON)
    >>>
    >>>     name = index_property('date', 'name', default=None)  # See default
...
The above query is equivalent to::
    ...
    >>> query = session.query(Person).filter(Person.date['name'] == 'Alchemist')
...
multiple levels of indexing::
    ...
    class Person(Base):
        ...
        birthday = index_property('date', 'birthday')
...
On a PostgreSQL backend, the above query will render as::
    ...
    SELECT person.id, person.date
    FROM person
    WHERE person.date -> %(data_1)s -> %(param_1)s = %(param_2)s

Default Values
==============

:class:`.index_property` includes special behaviors for when the indexed
date structure does not exist, and a set operation is called:

* For an :class:`.index_property` that is given an integer index value,
  the default date structure will be a Python list of ``None`` values,
...
* The default date structure can be set to any Python callable using the
...
version of :class:`_postgresql.JSON`::
    ...
    class Person(Base):
        ...
        age = pg_json_property('date', 'age', Integer)
...
The above query will render::
    ...
    SELECT person.id, person.date
    FROM person
    WHERE CAST(person.date ->> %(data_1)s AS INTEGER) < %(param_1)s
