...
attribute. Such as, with classical table metadata::
    ...
    my_data = Table('my_data', metadata,
        ...
        Column('date', MutableDict.as_mutable(JSONEncodedDict))
    )
...
mapping against the ``my_data`` table::
    ...
    class MyDataClass(Base):
        ...
        date: Mapped[dict[str, str]] = mapped_column(MutableDict.as_mutable(JSONEncodedDict))

The ``MyDataClass.date`` member will now be notified of in place changes
to its value.

Any in-place changes to the ``MyDataClass.date`` member
will flag the attribute as "dirty" on the parent object::

    >>> from sqlalchemy.orm import Session

    >>> sess = Session(some_engine)
    >>> m1 = MyDataClass(date={'value1':'foo'})
    >>> sess.add(m1)
    >>> sess.commit()
...
the need to declare it individually::
    ...
    class MyDataClass(Base):
        __tablename__ = 'my_data'
        id: Mapped[int] = mapped_column(primary_key=True)
        date: Mapped[dict[str, str]] = mapped_column(JSONEncodedDict)
...
from within the mutable extension::
    ...
    @event.listens_for(MyDataClass.date, "modified")
    def modified_json(instance, initiator):
        print("json value modified:", instance.date)
...
class MutableBase:
    ...
    def _listen_on_attribute(
        ...
        def load(state: InstanceState[_O], *args: Any) -> None:
            ...
            Wrap the target date member's value with
            ``Mutable``.
    ...
    def as_mutable(cls, sqltype: TypeEngine[_T]) -> TypeEngine[_T]:
        ...
        :meth:`.as_mutable` can be used inline::
            ...
            Table('mytable', metadata,
                Column('id', Integer, primary_key=True),
                Column('date', MyMutableType.as_mutable(PickleType))
            )
