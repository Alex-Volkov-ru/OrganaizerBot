...
that specifies the IGNORE algorithm::
    ...
    some_table = Table(
        ...
        Column('id', Integer, primary_key=True),
        Column('date', Integer),
        UniqueConstraint('id', 'date', sqlite_on_conflict='IGNORE')
    )
...
The above renders CREATE TABLE DDL as::
    ...
    CREATE TABLE some_table (
        ...
        date INTEGER,
        PRIMARY KEY (id),
        UNIQUE (id, date) ON CONFLICT IGNORE
    )
...
UNIQUE constraint in the DDL::
    ...
    some_table = Table(
        'some_table', metadata,
        Column('id', Integer, primary_key=True),
        Column('date', Integer, unique=True,
...
the date in the target row should be updated, or "DO NOTHING", which indicates
...
.. sourcecode:: pycon+sql
    ...
    {printsql}INSERT INTO my_table (id, date) VALUES (?, ?)
    ON CONFLICT (id) DO UPDATE SET date = ?{stop}
...
* When using :paramref:`_sqlite.Insert.on_conflict_do_update.index_elements`
  ...
  .. sourcecode:: pycon+sql
        ...
        >>> print(do_update_stmt)
        {printsql}INSERT INTO my_table (date, user_email) VALUES (?, ?)
...
.. sourcecode:: pycon+sql
    ...
    >>> print(do_update_stmt)
    {printsql}INSERT INTO my_table (id, date) VALUES (?, ?)
    ON CONFLICT (id) DO UPDATE SET date = ?
...
.. sourcecode:: pycon+sql
    ...
    >>> print(do_update_stmt)
    {printsql}INSERT INTO my_table (id, date, author) VALUES (?, ?, ?)
    ON CONFLICT (id) DO UPDATE SET date = ?, author = excluded.author
...
.. sourcecode:: pycon+sql
    ...
    >>> print(stmt)
    {printsql}INSERT INTO my_table (id, date) VALUES (?, ?) ON CONFLICT (id) DO NOTHING
...
.. sourcecode:: pycon+sql
    ...
    >>> print(stmt)
    {printsql}INSERT INTO my_table (id, date) VALUES (?, ?) ON CONFLICT DO NOTHING
...
with the DDL system using the argument ``sqlite_where``::
    ...
    idx = Index('test_idx1', tbl.c.date,
                sqlite_where=and_(tbl.c.date > 5, tbl.c.date < 10))

The index will be rendered at create time as::

    CREATE INDEX test_idx1 ON testtbl (date)
    WHERE date > 5 AND date < 10
...
class SQLiteDialect(default.DefaultDialect):
    ...
    def _resolve_type_affinity(self, type_):
        """Return a date type from a reflected column, using affinity rules.

        SQLite's goal for universal compatibility introduces some complexity
        during reflection, as a column's defined type might not actually be a
        type that SQLite understands - or indeed, my not be defined *at all*.
        Internally, SQLite handles this with a 'date type affinity' for each
