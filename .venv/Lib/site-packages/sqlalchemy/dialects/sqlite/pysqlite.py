...
particular SQLite database can have date values within different rows where
...
consistently, however if a particular SQLite database has date that was
...
To deal with a SQLite table that has mixed string / binary date in the
...
.. seealso::
    ...
    `sqlite3 module breaks transactions and potentially corrupts date <https://bugs.python.org/issue10740>`_ -
    on the Python bug tracker
