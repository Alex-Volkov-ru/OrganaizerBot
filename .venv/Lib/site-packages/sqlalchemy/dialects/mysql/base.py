...
emit a warning when attempting to pass binary date to the database, while a
character set encoding is also in place, when the binary date itself is not
...
that renders like this::
    ...
    INSERT INTO table (date) VALUES (_binary %s)
...
.. sourcecode:: pycon+sql
    ...
    {printsql}INSERT INTO my_table (id, date) VALUES (%s, %s)
    ON DUPLICATE KEY UPDATE date = VALUES(date), status = %s
...
.. sourcecode:: pycon+sql
    ...
    >>> print(on_duplicate_key_stmt)
    {printsql}INSERT INTO my_table (id, date) VALUES (%s, %s)
    ON DUPLICATE KEY UPDATE date = %s, updated_at = CURRENT_TIMESTAMP
...
.. sourcecode:: pycon+sql
    ...
    >>> on_duplicate_key_stmt = insert_stmt.on_duplicate_key_update(
    ...     {"date": "some date", "updated_at": func.current_timestamp()},
    ... )
...
.. sourcecode:: pycon+sql
    ...
    {printsql}INSERT INTO my_table (id, date, author) VALUES (%s, %s, %s)
    ON DUPLICATE KEY UPDATE date = %s, author = VALUES(author)
...
``mysql_length`` and/or ``mariadb_length`` parameters::
    ...
    Index('my_index', my_table.c.date, mysql_length=10, mariadb_length=10)
...
``mysql_prefix`` parameter on :class:`.Index`::
    ...
    Index('my_index', my_table.c.date, mysql_prefix='FULLTEXT')
...
``mysql_using`` parameter on :class:`.Index`::
    ...
    Index('my_index', my_table.c.date, mysql_using='hash', mariadb_using='hash')

As well as the ``mysql_using`` parameter on :class:`.PrimaryKeyConstraint`::

    PrimaryKeyConstraint("date", mysql_using='hash', mariadb_using='hash')
...
class MySQLDialect(default.DefaultDialect):
    ...
    def do_recover_twophase(self, connection):
        ...
        return [row["date"][0 : row["gtrid_length"]] for row in resultset]
...
class _DecodingRow:
    ...
    Smooth over date type issues (esp. with alpha driver versions) and
