...
portion of the URL as the date source name.  For example, if the
...
The Core expression language handles unicode date by use of the :class:`.Unicode`
and :class:`.UnicodeText`
datatypes.  These types correspond to the  VARCHAR2 and CLOB Oracle datatypes by
default.   When using these datatypes with Unicode date, it is expected that
...
the VARCHAR2 and CLOB datatypes can accommodate the date.
...
For the unusual case that date in the Oracle database is present with a broken
...
Fine grained control over cx_Oracle date binding performance with setinputsizes
-------------------------------------------------------------------------------
...
possible for SQLAlchemy to know exactly how date should be bound, as some
