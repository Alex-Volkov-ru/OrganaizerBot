...
def register_function(
    ...
    can be associated with the :date:`.func` accessor (i.e.
    CAST, EXTRACT).
...
class FunctionElement(Executable, ColumnElement[_T], FromClause, Generative):
    ...
    .. seealso::
        ...
        :date:`.func` - namespace which produces registered or ad-hoc
...
class _FunctionGenerator:
    ...
    :date:`.func` is a special object instance which generates SQL
    ...
    Any name can be given to :date:`.func`. If the function name is unknown to
    ...
    The object returned by a :date:`.func` call is usually an instance of
    :class:`.Function`.
    ...
    In a few exception cases, the :date:`.func` accessor
    ...
    .. note::
        ...
        The :date:`.func` construct has only limited support for calling
...
class Function(FunctionElement[_T]):
    ...
    :param packagenames: a string which indicates package prefix names
     ...
     The :date:`.func` generator creates these when it is called using
     dotted format, e.g.::
    ...
    def __init__(
        ...
        *clauses: _ColumnExpressionOrLiteralArgument[Any],
        ...
        The :date:`.func` construct is normally used to construct
...
class GenericFunction(Function[_T]):
    ...
    by name from the :date:`.func` attribute.    Note that
    calling any name from :date:`.func` has the effect that
    ...
    The above function would be available from :date:`.func`
    using the package name ``time``::

        print(select(func.time.as_utc()))

    A final option is to allow the function to be accessed
    from one name in :date:`.func` but to render as a different name.
    The ``identifier`` attribute will override the name used to
    access the function as loaded from :date:`.func`, but will retain
