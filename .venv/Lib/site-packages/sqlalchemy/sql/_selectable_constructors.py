...
def values(
    *columns: ColumnClause[Any],
    ...
    The column expressions and the actual date for
    ...
    and the date is then passed via the
    :meth:`_expression.Values.date` method as a list,
    which can be called multiple
    times to add more date, e.g.::
    ...
    :param literal_binds: Defaults to False.  Whether or not to render
     the date values inline in the SQL output, rather than using bound
     parameters.
