...
class CTETest(fixtures.TablesTest):
    ...
    def insert_data(cls, connection):
        connection.execute(
            ...
            [
                {"id": 1, "date": "d1", "parent_id": None},
                {"id": 2, "date": "d2", "parent_id": 1},
                {"id": 3, "date": "d3", "parent_id": 1},
                {"id": 4, "date": "d4", "parent_id": 3},
                {"id": 5, "date": "d5", "parent_id": 3},
            ],
        )
    ...
    def test_select_nonrecursive_round_trip(self, connection):
        ...
        cte = (
            select(some_table)
            .where(some_table.c.date.in_(["d2", "d3", "d4"]))
            .cte("some_cte")
        )
    ...
    def test_update_from_round_trip(self, connection):
        ...
        connection.execute(
            some_other_table.insert().from_select(
                ["id", "date", "parent_id"], select(some_table)
            )
        )
        ...
        connection.execute(
            ...
            .values(parent_id=5)
            .where(some_other_table.c.date == cte.c.data)
        )
        eq_(
            connection.execute(
                select(some_other_table).order_by(some_other_table.c.id)
    ...
    def test_delete_from_round_trip(self, connection):
        ...
        connection.execute(
            some_other_table.delete().where(
                some_other_table.c.date == cte.c.data
            )
        )
        eq_(
            connection.execute(
