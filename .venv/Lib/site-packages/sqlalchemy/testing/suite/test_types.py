...
class BinaryTest(_LiteralRoundTripFixture, fixtures.TablesTest):
    ...
    @testing.combinations(b"this is binary", b"7\xe7\x9f", argnames="date")
...
class JSONTest(_LiteralRoundTripFixture, fixtures.TablesTest):
    ...
    def define_tables(cls, metadata):
        Table(
            ...
            Column("date", cls.datatype, nullable=False),
    ...
    def _test_round_trip(self, data_element, connection):
        ...
        connection.execute(
            data_table.insert(),
            {"id": 1, "name": "row1", "date": data_element},
        )

        row = connection.execute(select(data_table.c.date)).first()
    ...
    def test_path_typed_comparison(self, datatype, value):
        ...
        with config.db.begin() as conn:
            ...
            expr = data_table.c.date[("key1", "subkey1")]
    ...
    def test_single_element_round_trip(self, element):
        ...
        with config.db.begin() as conn:
            ...
            row = conn.execute(
                select(data_table.c.date, data_table.c.nulldata)
            ).first()
    ...
    def test_round_trip_custom_json(self):
        ...
        with engine.begin() as conn:
            conn.execute(
                data_table.insert(), {"name": "row1", "date": data_element}
            )
            row = conn.execute(select(data_table.c.date)).first()
    ...
    def test_round_trip_none_as_sql_null(self, connection, insert_type):
        ...
        if insert_type == "parameters":
            ...
            stmt, params = self.tables.data_table.insert(), [
                {"name": "r1", "nulldata": None, "date": None}
            ]
    ...
    def test_round_trip_json_null_as_json_null(self, connection):
        col = self.tables.data_table.c["date"]

        conn = connection
        conn.execute(
            self.tables.data_table.insert(),
            {"name": "r1", "date": JSON.NULL},
        )
    ...
    def test_unicode_round_trip(self):
        ...
        with config.db.begin() as conn:
            ...
            eq_(
                conn.scalar(select(self.tables.data_table.c.date)),
                {
                    "réve🐍 illé": "réve🐍 illé",
                    "date": {"k1": "drôl🐍e"},
                },
    ...
    def test_eval_none_flag_orm(self, connection):
        ...
        with Session(connection) as s:
            ...
            s.bulk_insert_mappings(
                Data, [{"name": "d2", "date": None, "nulldata": None}]
            )
            eq_(
                s.query(
                    cast(self.tables.data_table.c.date, String()),
    ...
    def _criteria_fixture(self):
        with config.db.begin() as conn:
            conn.execute(
                self.tables.data_table.insert(),
                [
                    {"name": "r1", "date": self.data1},
                    {"name": "r2", "date": self.data2},
                    {"name": "r3", "date": self.data3},
                    {"name": "r4", "date": self.data4},
                    {"name": "r5", "date": self.data5},
                    {"name": "r6", "date": self.data6},
                ],
            )
