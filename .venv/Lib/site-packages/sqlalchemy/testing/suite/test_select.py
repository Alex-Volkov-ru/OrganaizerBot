...
class CollateTest(fixtures.TablesTest):
    ...
    def insert_data(cls, connection):
        connection.execute(
            ...
            [
                {"id": 1, "date": "collate data1"},
                {"id": 2, "date": "collate data2"},
            ],
        )
    ...
    def test_collate_order_by(self):
        ...
        self._assert_result(
            select(self.tables.some_table).order_by(
                self.tables.some_table.c.date.collate(collation).asc()
            ),
...
class LikeFunctionsTest(fixtures.TablesTest):
    ...
    def test_startswith_unescaped(self):
        col = self.tables.some_table.c.date
...
class ExistsTest(fixtures.TablesTest):
        ...
        eq_(
            connection.execute(
                select(literal(1)).where(
                    exists().where(stuff.c.date == "some date")
                )
            ).fetchall(),
            [(1,)],
        )
    ...
    def test_select_exists_false(self, connection):
        ...
        eq_(
            connection.execute(
                select(literal(1)).where(
                    exists().where(stuff.c.date == "no date")
                )
            ).fetchall(),
            [],
        )
