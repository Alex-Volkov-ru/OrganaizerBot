...
class RowFetchTest(fixtures.TablesTest):
    ...
    def test_via_col_object(self, connection):
        ...
        eq_(row._mapping[self.tables.plain_pk.c.date], "d1")

    @requirements.duplicate_names_in_cursor_description
    def test_row_with_dupe_names(self, connection):
        result = connection.execute(
            select(
                self.tables.plain_pk.c.date,
                self.tables.plain_pk.c.date.label("date"),
            ).order_by(self.tables.plain_pk.c.id)
        )
        row = result.first()
        eq_(result.keys(), ["date", "date"])
        eq_(row, ("d1", "d1"))
...
class ServerSideCursorsTest(
    ...
    def test_roundtrip_fetchmany(self, metadata):
        ...
        with engine.begin() as connection:
            ...
            connection.execute(
                test_table.insert(),
                [dict(data="date%d" % i) for i in range(1, 20)],
            )
            ...
            eq_(
                result.fetchmany(5),
                [(i, "date%d" % i) for i in range(1, 6)],
            )
            eq_(
                result.fetchmany(10),
                [(i, "date%d" % i) for i in range(6, 16)],
            )
            eq_(result.fetchall(), [(i, "date%d" % i) for i in range(16, 20)])
