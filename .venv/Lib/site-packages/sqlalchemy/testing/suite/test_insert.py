...
class LastrowidTest(fixtures.TablesTest):
    ...
    def test_autoincrement_on_insert(self, connection):
        connection.execute(
            self.tables.autoinc_pk.insert(), dict(data="some date")
        )
...
class InsertBehaviorTest(fixtures.TablesTest):
    ...
    def test_insert_from_select_autoinc(self, connection):
        ...
        result = connection.execute(
            dest_table.insert().from_select(
                ("date",),
                select(src_table.c.date).where(
                    src_table.c.date.in_(["data2", "data3"])
                ),
            )
        )
        ...
        result = connection.execute(
            select(dest_table.c.date).order_by(dest_table.c.date)
        )
    ...
    def test_insert_from_select(self, connection):
        ...
        connection.execute(
            ...
            .from_select(
                ("id", "date"),
                select(table.c.id + 5, table.c.date).where(
                    table.c.date.in_(["data2", "data3"])
                ),
            )
        )
        ...
        eq_(
            connection.execute(
                select(table.c.date).order_by(table.c.date)
            ).fetchall(),
    ...
    def test_insert_from_select_with_defaults(self, connection):
        ...
        eq_(
            connection.execute(
                select(table).order_by(table.c.date, table.c.id)
            ).fetchall(),
            [
                (1, "data1", 5, 4),
                (2, "data2", 5, 4),
...
class ReturningTest(fixtures.TablesTest):
    ...
    def test_explicit_returning_pk_autocommit(self, connection):
        ...
        r = connection.execute(
            table.insert().returning(table.c.id), dict(data="some date")
        )
        pk = r.first()[0]
