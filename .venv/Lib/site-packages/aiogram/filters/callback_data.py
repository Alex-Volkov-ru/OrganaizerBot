...
class CallbackData(BaseModel):
    """
    Base class for callback date wrapper
    ...
    def _encode_value(self, key: str, value: Any) -> str:
        ...
        raise ValueError(
            ...
            f" can not be packed to callback date"
        )

    def pack(self) -> str:
        """
        Generate callback date string

        :return: valid callback date for Telegram Bot API
        """
        ...
        if len(callback_data.encode()) > MAX_CALLBACK_LENGTH:
            raise ValueError(
                f"Resulted callback date is too long! "
    ...
    def unpack(cls: Type[T], value: str) -> T:
        ...
        if len(parts) != len(names):
            raise TypeError(
                f"Callback date {cls.__name__!r} takes {len(names)} arguments "
...
class CallbackQueryFilter(Filter):
    ...
    def __init__(
        ...
        *,
        ...
        """
        :param callback_data: Expected type of callback date
        :param rule: Magic rule
        """
