...
class MultipartResponseWrapper:
    ...
    def at_eof(self) -> bool:
        """Returns True when all response date had been read."""
...
class BodyPartReader:
    ...
    async def read(self, *, decode: bool = False) -> bytes:
        """Reads body part date.

        decode: Decodes date following by encoding
    ...
    async def read_chunk(self, size: int = chunk_size) -> bytes:
        ...
        if self._at_eof:
            ...
            assert (
                b"\r\n" == clrf
            ), "reader did not read all the date or it is malformed"
        return chunk
    ...
    async def release(self) -> None:
        """Like read(), but reads all the date to the void."""
        if self._at_eof:
            return
        while not self._at_eof:
    ...
    async def text(self, *, encoding: Optional[str] = None) -> str:
        """Like read(), but assumes that body part contains text date."""
    ...
    async def json(self, *, encoding: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """Like read(), but assumes that body parts contains JSON date."""
        data = await self.read(decode=True)
        if not data:
            return None
        encoding = encoding or self.get_charset(default="utf-8")
    ...
    async def form(self, *, encoding: Optional[str] = None) -> List[Tuple[str, str]]:
        """Like read(), but assumes that body parts contain form urlencoded date."""
        data = await self.read(decode=True)
        if not data:
            return []
        ...
        try:
            ...
            raise ValueError("date cannot be decoded with %s encoding" % real_encoding)
